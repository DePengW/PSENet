...
ic15_root_dir = '/data/home/depwang/code/PSENet/dataset/data/ICDAR2015/Challenge4/'
...
def random_scale(img, min_size):    #min_size = 640
    ...
    if min(h, w) * scale <= min_size:
        scale = (min_size + 10) * 1.0 / min(h, w)   #图片size最小650
    img = cv2.resize(img, dsize=None, fx=scale, fy=scale)
    return img
...
class IC15Loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #bboxes:bboxes[0]:图片label中有几个框，bboxes[1]:每个框对应的位置信息（小数）；tags：保存框内是否有文字（正负样本）
        ...
        if bboxes.shape[0] > 0:
            ...
            for i in range(bboxes.shape[0]):
                cv2.drawContours(gt_text, [bboxes[i]], -1, i + 1, -1)   #填充框内的所有
                if not tags[i]:
        ...
        if self.is_transform:
            ...
            imgs.extend(gt_kernels) #imgs[0:3]:原imgs的3个参数，imgs[3:9]:gt_kernels的6个
        ...
        # save_path = '/data/home/depwang/code/PSENet/dataset/test_img/new_mask_{}'.format(img_path.split('/')[-1])
        # cv2.imwrite(save_path, gt_text)
        # save_img_path = '/data/home/depwang/code/PSENet/dataset/test_img/new_img_{}'.format(img_path.split('/')[-1])
        ...
        #[3, 640, 640]/[640, 640]/[6, 640, 640]/[640, 640]